...
short_data = "Hello! My name is Ray. How are you?"
...
short_data_embeddings = SentenceTransformer('all-MiniLM-L6-v2').encode(short_data).tolist() #for pc
long_data_embeddings = SentenceTransformer('all-MiniLM-L6-v2').encode(long_data_chunked).tolist() #for pc
...
pc = Pinecone(api_key="a0788762-ee12-48b5-809b-b7fb891d54f1")
...
text = "hello"
    ...
    remember_one_pc_time = timeit.timeit('remember_pc(index, text)', 
                        setup='''
from pinecone import Pinecone, PodSpec
from sentence_transformers import SentenceTransformer
import os
...
text = SentenceTransformer('all-MiniLM-L6-v2').encode("hello").tolist()
                        ''',
                        number=num_executions) / num_executions
...
text = "civil law"
    ...
    remember_many_pc_time = timeit.timeit('remember_pc(index, text)', 
                        setup='''
from pinecone import Pinecone, PodSpec
from sentence_transformers import SentenceTransformer
import os
...
text = SentenceTransformer('all-MiniLM-L6-v2').encode("civil law").tolist()
                        ''',
                        number=num_executions) / num_executions
